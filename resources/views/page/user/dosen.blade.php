                ...
                <tbody>
                    @foreach ($data as $key)
                        <tr>
                            ...
                            <td>{{$key->email}}</td>
                            <td>
                                <button type="button" class="btn btn-sm btn-warning mb-1" onclick="editDosen({{$key->id}})"><i class="fa fa-user-edit mr-1"></i>Ubah</button><br>
                                <button type="button" class="btn btn-sm btn-danger mb-1" onclick="hapusDosen({{$key->id}})"><i class="fa fa-trash mr-1"></i>Hapus</button>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <script>
        ...
        function editDosen(id){
            ...
            $.post("/user/dosen/read", {id: id}, function(data, status){
                var message = '<form id="edit_dosen"><input type="hidden" name="nip_asal" value="' + data.id + '"><input type="hidden" name="email_asal" value="' + data.email + '">';
                ...
                $(document).ready(function(){
                    $("#edit_dosen").submit(function(event){
                        event.preventDefault();
                        $('#ubah_dosen_btn_loading').show();
                        $('#ubah_dosen_btn_submit').hide();
                        ...
                        $.ajax({
                            ...
                            processData: false,
                            success:function(data){
                                $('#ubah_dosen_btn_loading').hide();
                                $('#ubah_dosen_btn_submit').show();
